...
bool SampleApp::OnInit()
{
    // メッシュをロード.
    {
        ...
        for (size_t i = 0; i < resMesh.size(); ++i)
        {
            ...
            // 初期化処理.
            if (!mesh->Init(m_pDevice.Get(), m_pPool[POOL_TYPE_RES], resMesh[i], batch))
            {
    ...
    // シーン用ルートシグニチャの生成.
    {
        ...
        desc.Begin(15)
            .SetCBV(ShaderStage::MS,  0, 0)
            .SetCBV(ShaderStage::MS,  1, 1)
            .SetSRV(ShaderStage::MS,  2, 0)
            .SetSRV(ShaderStage::MS,  3, 1)
            .SetSRV(ShaderStage::MS,  4, 2)
            .SetSRV(ShaderStage::MS,  5, 3)
            .SetCBV(ShaderStage::PS,  6, 1)
            .SetCBV(ShaderStage::PS,  7, 2)
            .SetSRV(ShaderStage::PS,  8, 0)
            .SetSRV(ShaderStage::PS,  9, 1)
            .SetSRV(ShaderStage::PS, 10, 2)
            .SetSRV(ShaderStage::PS, 11, 3)
            .SetSRV(ShaderStage::PS, 12, 4)
            .SetSRV(ShaderStage::PS, 13, 5)
            .SetSRV(ShaderStage::PS, 14, 6)
    ...
    // シーン用パイプラインステートの生成.
    {
        ...
        // 頂点シェーダを検索.
        if (!SearchFilePath(L"BasicMS.cso", msPath))
        {
        ...
        // 頂点シェーダを読み込む.
        auto hr = D3DReadFileToBlob(msPath.c_str(), pMSBlob.GetAddressOf());
        if (FAILED(hr))
        {
            ELOG("Error : D3DReadFiledToBlob() Failed. path = %ls", msPath.c_str());
            return false;
        }
        ...
        rtvFormats.RTFormats[0]     = m_SceneColorTarget.GetDesc().Format;
        ...
        desc.MS                 = { pMSBlob->GetBufferPointer(), pMSBlob->GetBufferSize() };
        desc.PS                 = { pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize() };
        desc.RasterizerState    = DirectX::CommonStates::CullNone;
        desc.BlendState         = DirectX::CommonStates::Opaque;
        desc.DepthStencilState  = DirectX::CommonStates::DepthDefault;
        desc.SampleMask         = UINT_MAX;
        desc.RTFormats          = rtvFormats;
        desc.DSFormat           = m_SceneDepthTarget.GetDSVDesc().Format;
        ...
        // パイプラインステートを生成.
        hr = m_pDevice->CreatePipelineState(&streamDesc, IID_PPV_ARGS(m_pScenePSO.GetAddressOf()));
        if (FAILED(hr))
        {
...
//      シーンを描画します.
//-----------------------------------------------------------------------------
void SampleApp::DrawScene(ID3D12GraphicsCommandList6* pCmd)
{
    // ライトバッファの更新.
    {
    ...
    pCmd->SetGraphicsRootDescriptorTable(6,  m_LightCB    [m_FrameIndex].GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(7,  m_CameraCB   [m_FrameIndex].GetHandleGPU());
    pCmd->SetGraphicsRootDescriptorTable(8,  m_IBLBaker.GetHandleGPU_DFG());
    pCmd->SetGraphicsRootDescriptorTable(9,  m_IBLBaker.GetHandleGPU_DiffuseLD());
    pCmd->SetGraphicsRootDescriptorTable(10, m_IBLBaker.GetHandleGPU_SpecularLD());
...
//      メッシュを描画します.
//-----------------------------------------------------------------------------
void SampleApp::DrawMesh(ID3D12GraphicsCommandList6* pCmd, int material_index)
{
    MeshDescriptorSetting settings;
    settings.Vertices               = { 2, true };
    settings.UniqueVertexIndices    = { 3, true };
    settings.Meshlets               = { 4, true };
    settings.PrimitiveIndices       = { 5, true };
    ...
    for (size_t i = 0; i<m_pMesh.size(); ++i)
    {
        ...
        // テクスチャセットを設定.
        pCmd->SetGraphicsRootDescriptorTable(11, mat.GetTextureHandle(id, TU_BASE_COLOR));
        pCmd->SetGraphicsRootDescriptorTable(12, mat.GetTextureHandle(id, TU_METALLIC));
        pCmd->SetGraphicsRootDescriptorTable(13, mat.GetTextureHandle(id, TU_ROUGHNESS));
        pCmd->SetGraphicsRootDescriptorTable(14, mat.GetTextureHandle(id, TU_NORMAL));

        // メッシュを描画.
        m_pMesh[i]->Dispatch(pCmd, settings);
    }
}
...
//      トーンマップを適用します.
//-----------------------------------------------------------------------------
void SampleApp::DrawTonemap(ID3D12GraphicsCommandList6* pCmd)
{
    // 定数バッファ更新
    {
