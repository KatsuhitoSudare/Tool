...
private:
    ...
    ComPtr<ID3D12Device8>                  m_pDevice;                      // ★変更：デバイスです.
    ...
    ComPtr<ID3D12GraphicsCommandList6>     m_pCmdList;                     // ★変更：コマンドリストです.
    ComPtr<ID3D12DescriptorHeap>           m_pHeapRTV;                     // ディスクリプタヒープです(レンダーターゲットビュー).
    ComPtr<ID3D12Fence>                    m_pFence;                       // フェンスです.
    ComPtr<ID3D12DescriptorHeap>           m_pHeapRes;                     // ★変更:：ディスクリプタヒープです(定数バッファビュー・シェーダリソースビュー・アンオーダードアクセスビュー)
    ComPtr<ID3D12Resource>                 m_pVB;                          // 頂点バッファです.
    ComPtr<ID3D12Resource>                 m_pIB;                          // ★追加：インデックスバッファです.
